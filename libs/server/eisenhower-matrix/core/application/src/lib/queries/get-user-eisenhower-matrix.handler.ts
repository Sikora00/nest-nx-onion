...
export class GetUserEisenhowerMatrixHandler implements ICommandHandler<GetUserEisenhowerMatrixQuery> {
  constructor(private persistence: JsonFilesPersistence) {}
  async execute(command: GetUserEisenhowerMatrixQuery): Promise<EisenhowerMatrix> {
    const toDos = await this.persistence.getFileData<EisenhowerMatrix[]>();
    return toDos.find((toDo: EisenhowerMatrix) => toDo.userId === command.userId);
  }
}
